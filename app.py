...
# ───────────────────────────────────────────
# Статистика — считается в фоне из полной базы
# ───────────────────────────────────────────
def compute_stats():
    ...
    try:
        ...
        with app.app_context():
            _stats_cache = render_template('stats.html',
...
def global_stats():
    ...
    if not _stats_cache:
        return render_template('stats.html',
            ...
            total_likes=0, negative_count=0, mythic_count=0,
            legendary_count=0, epic_count=0, avg_objects=0, max_objects=0,
...
def level_page(level_id):
    ...
    if not data:
        ...
        conn.close()
        if row:
            return render_template('level.html', level=row_to_dict(row))
...
def random_level():
    conn = get_db()
    row = conn.execute('SELECT id FROM levels WHERE downloads > 1000 ORDER BY RANDOM() LIMIT 1').fetchone()
    conn.close()
    if row:
        return redirect(f'/level/{row["id"]}')
    return redirect('/')
...
def search_levels_api():
    ...
    if not query:
        return jsonify([])
    return jsonify(search_levels(query))
...
def api_levels():
    ...
    rows = conn.execute(f'''
        SELECT * FROM levels
        WHERE 1=1 {dl_filter} {period_filter}
...
def leaderboard_page():
    conn = get_db()
    try:
        latest_date = conn.execute('SELECT MAX(date) FROM player_history').fetchone()[0]
    except:
        conn.close()
        return render_template('leaderboard.html', players=[], dates=[], updated=None)
    if not latest_date:
        conn.close()
        return render_template('leaderboard.html', players=[], dates=[], updated=None)
    top = conn.execute("""
        ...
        WHERE ph.date = ? ORDER BY ph.rank ASC LIMIT 100
    """, (latest_date,)).fetchall()
    dates = [r['date'] for r in reversed(conn.execute(
        "SELECT DISTINCT date FROM player_history ORDER BY date DESC LIMIT 30"
    ).fetchall())]
    conn.close()
    return render_template('leaderboard.html', players=[dict(r) for r in top], dates=dates, updated=latest_date)
...
def leaderboard_history_api():
    ...
    if accounts:
        ...
        aid_list = [r['account_id'] for r in conn.execute(
            "SELECT account_id FROM player_history WHERE date=? ORDER BY rank ASC LIMIT 10", (latest,)
        ).fetchall()]
    result = {}
    for aid in aid_list:
        u = conn.execute('SELECT username FROM players WHERE account_id=?', (aid,)).fetchone()
        if not u:
            continue
        history = conn.execute(
            f"SELECT date, {metric} as value FROM player_history WHERE account_id=? ORDER BY date ASC", (aid,)
        ).fetchall()
        result[aid] = {'username': u['username'], 'data': {r['date']: r['value'] for r in history}}
    conn.close()
    return jsonify({'dates': dates, 'players': result})

@app.route('/api/level/<int:level_id>/history')
def level_history_api(level_id):
    conn = get_full_db()
    try:
        rows = conn.execute(
            'SELECT * FROM level_history WHERE level_id=? ORDER BY time ASC', (level_id,)
        ).fetchall()
    except:
        rows = []
    conn.close()
